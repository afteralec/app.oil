...
const createActorImagePlayerProperties = `-- name: CreateActorImagePlayerProperties :execresult
INSERT INTO actor_images_player_properties (aiid, pid) VALUES (?, ?)
`

type CreateActorImagePlayerPropertiesParams struct {
	AIID int64
	PID  int64
}

func (q *Queries) CreateActorImagePlayerProperties(ctx context.Context, arg CreateActorImagePlayerPropertiesParams) (sql.Result, error) {
	return q.exec(ctx, q.createActorImagePlayerPropertiesStmt, createActorImagePlayerProperties, arg.AIID, arg.PID)
}
...
const getActorImagePlayerPropertiesForImage = `-- name: GetActorImagePlayerPropertiesForImage :one
SELECT created_at, updated_at, aiid, pid, id, current FROM actor_images_player_properties WHERE aiid = ?
`

func (q *Queries) GetActorImagePlayerPropertiesForImage(ctx context.Context, aiid int64) (ActorImagesPlayerProperty, error) {
	row := q.queryRow(ctx, q.getActorImagePlayerPropertiesForImageStmt, getActorImagePlayerPropertiesForImage, aiid)
	var i ActorImagesPlayerProperty
	err := row.Scan(
		&i.CreatedAt,
		&i.UpdatedAt,
		&i.AIID,
...
const setActorImagePlayerPropertiesCurrent = `-- name: SetActorImagePlayerPropertiesCurrent :exec
UPDATE actor_images_player_properties SET current = ? WHERE id = ?
`

type SetActorImagePlayerPropertiesCurrentParams struct {
	Current bool
	ID      int64
}

func (q *Queries) SetActorImagePlayerPropertiesCurrent(ctx context.Context, arg SetActorImagePlayerPropertiesCurrentParams) error {
	_, err := q.exec(ctx, q.setActorImagePlayerPropertiesCurrentStmt, setActorImagePlayerPropertiesCurrent, arg.Current, arg.ID)
	return err
}
